...
def compute_prediction_intervals(
    y_pred_point_log, q, min_physical_price=42000.0, max_physical_price=525000.0
):
    ...
    y_pred_point = np.clip(
        y_pred_point, a_min=min_physical_price, a_max=max_physical_price
    )
    lower_bound = np.clip(
        lower_bound, a_min=min_physical_price, a_max=max_physical_price
    )
    upper_bound = np.clip(
        upper_bound, a_min=min_physical_price, a_max=max_physical_price
    )
